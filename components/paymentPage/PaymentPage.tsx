  ...
  const handleConfirmPayment = async (transactionId: string, fileUrl: string) => {
    if (paymentData.planCategory === 'kitty') {
      const payload = {
        ...
        proofImage: fileUrl,
        transactionType: "KITTY" as const
      }
    ...
    } else if (paymentData.planCategory === 'gold') {
      const payload = {
        amount: paymentData.investmentAmount,
        transactionId: transactionId,
        proofImage: fileUrl,
        transactionType: "GOLD_BUYER" as const
      }
      const res = await createGoldInvestmentApi(payload)
      ...
      const payload = {
        ...
        transactionId: transactionId,
        proofImage: fileUrl,
        transactionType: "GOLD_INVESTOR" as const
      }
      // const res = await QRTransectionApi(payload)
